...
class AnalyticsBarChart extends React.Component<AnalyticsBarChartProps, {}> {
  render() {
  ...
  createBars() {
    ...
    const bars = chartData.map((chartItem, index) => {
      ...
      return <PNLBar key={index} index={index} basePnl={basePnl} symbol={symbol} currencyPair={currencyPair} isPnL={isPnL} maxVal={maxWidth} />
    })
    return bars
  }
}
