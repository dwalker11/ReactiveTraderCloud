...
export default class Blotter extends React.Component<BlotterProps, {}> {
  ...
  createGridColumns(trades: Trade[], currencyPairs: CurrencyPair[]): any[] {
    return [
      <Column key="tradeId" dataKey="tradeId" label={'Id'} cellRenderer={this.baseCellRenderer('tradeId')} flexGrow={1} width={50} />,
      <Column
        key="Date"
        dataKey="Date"
        label={'Date'}
        ...
        width={150}
      />,
      <Column key="Dir" dataKey="Dir" label={'Direction'} cellRenderer={this.baseCellRenderer('direction')} flexGrow={1} width={80} />,
      <Column key="CCY" dataKey="CCY" label={'CCYCCY'} cellRenderer={this.baseCellRenderer('symbol')} flexGrow={1} width={70} />,
      <Column
        key="Notional"
        dataKey="Notional"
        label={'Notional'}
        cellRenderer={(props: any) =>
          this.customCellRenderer(this.getFormattedNotional(trades[props.rowIndex]), getCellClassName(trades[props.rowIndex].status, 'Notional'))(props)
        }
        flexGrow={1}
        width={120}
      />,
      ...
      <Column
        ...
        label={'Status'}
        cellRenderer={(props: any) =>
          this.baseCellRenderer('status', classNames('blotter__trade-status', getTradeStatusCellStyle(trades[props.rowIndex].status)))(props)
        }
        flexGrow={1}
        width={80}
      />,
      <Column
        ...
        width={100}
      />,
      <Column key="Trader" dataKey="Trader" label={'Trader'} cellRenderer={this.baseCellRenderer('traderName')} flexGrow={1} width={80} />
    ]
  }
